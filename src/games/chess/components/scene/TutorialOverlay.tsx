 ...
 * board while tutorial mode is active.
 *
 * Contains two layers:
 *  - Highlights: soft tinted square outlines for reachable/relevant squares.
 *  - Arrows: directional indicators for guiding attention (opening moves,
 *    explanation of a capture, etc.)
 *
 * All overlays are world-space meshes rendered in the same R3F tree as the
 * rest of ChessScene, so they participate in camera transforms naturally.
 */
export function TutorialOverlay() {
  const isActive = useTutorialStore((s) => s.isActive);
  const highlights = useTutorialStore((s) => s.highlights);
  const arrows = useTutorialStore((s) => s.arrows);
